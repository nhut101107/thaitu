...
export function adminView() {
  ...
  return `<div class="page admin-page">
    ...
    <details class="admin-section"><summary><span><i>🏆</i><b>Bảng xếp hạng referral</b><small>Tuần / tháng, tự chốt chu kỳ và thưởng</small></span><em>⌄</em></summary><div class="admin-section-body">${(data.leaderboards || []).map((board) => `<article class="admin-row"><div><b>${board.period}</b><small>Top ${board.items.length} · ${board.settings.rewardEnabled ? `thưởng ${board.settings.rewardCredits}` : "đang tắt thưởng"}</small></div></article>`).join("")}</div></details>
    ...
    <details class="admin-section" open><summary><span><i>▣</i><b>Reports doanh thu</b><small>Database thật, lọc và xuất CSV UTF-8</small></span><em>⌄</em></summary><div class="admin-section-body"><div class="report-summary"><b>${formatMoney(data.reports?.summary?.revenue || 0)}</b><span>${data.reports?.summary?.orders || 0} đơn · giảm ${formatMoney(data.reports?.summary?.discounts || 0)}</span></div><a class="button wide" href="/api/admin/reports?format=csv">Tải CSV báo cáo</a></div></details>
