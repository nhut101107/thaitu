...
export function adminView() {
  ...
  return `<div class="page admin-page">
    ...
    <details class="admin-section"><summary><span><i>💸</i><b>Duyệt nạp tiền</b><small>${s.pendingDeposits} yêu cầu đang chờ</small></span><em>⌄</em></summary><div class="admin-section-body">${adminRows(data.transactions, (tx) => `<article class="admin-row transaction ${tx.status.toLowerCase()}"><div><b>#${tx.id} · ${formatMoney(tx.amount)}</b><small>ID ${tx.user_id}${tx.username ? ` · @${escapeHtml(tx.username)}` : ""} · ${escapeHtml(tx.status)}</small></div>${tx.status === "PENDING" ? `<span><button class="approve" data-admin-tx="${tx.id}" data-status="APPROVED">Duyệt</button><button class="reject" data-admin-tx="${tx.id}" data-status="REJECTED">Từ chối</button></span>` : ""}</article>`, "Chưa có giao dịch nạp.")}</div></details>
...
export function bindAdminEvents() {
  ...
  document.querySelectorAll("[data-admin-tx]").forEach((button) => button.onclick = async () => { try { await api.adminUpdateTransaction(button.dataset.adminTx, button.dataset.status); await loadAdmin(); toast("Đã xử lý giao dịch"); } catch (error) { toast(error.message, "error"); } });
