...
import {emptyState, escapeHtml, formatMoney, modal, skeleton, toast} from "./components.js?v=19";
import {translateText} from "./i18n.js?v=3";

export async function loadAdmin(query = "") {
  try {
    const [admin, reports, flashSales, leaderboards, missions, operations] = await Promise.all([api.adminDashboard(query), api.adminReports(), api.adminFlashSales(), api.adminReferralLeaderboards(), api.adminMissions(), api.adminOperations()]);
...
export function adminView() {
  ...
  const trial = data.settings.trial || {nftokenEnabled:false,nftokenDailyLimit:0,cookieEnabled:false,cookieDailyLimit:0};
  const operationSettings = data.settings.operations || {deliveryWarrantyDays:1,lowStockThreshold:5};
  return `<div class="page admin-page">
    ...
    <div class="admin-stats">
      ...
      <article><i>♢</i><small>Bảo hành chờ xử lý</small><b>${s.pendingWarranties || 0}</b></article>
      <article><i>⚠</i><small>Kho cách ly / User rủi ro</small><b>${s.quarantinedStock || 0} / ${s.riskUsers || 0}</b></article>
    </div>

    <details class="admin-section" open><summary><span><i>⚙️</i><b>Vận hành hệ thống</b><small>Bảo trì, trải nghiệm và bật/tắt chức năng</small></span><em>⌄</em></summary><div class="admin-section-body"><form data-admin-settings><label class="danger-check"><input name="maintenance" type="checkbox" ${data.settings.maintenance ? "checked" : ""}> Bật chế độ bảo trì cho khách</label><label class="field">Thông báo trên trang chủ<textarea name="announcement" maxlength="500" placeholder="Để trống nếu không có thông báo">${escapeHtml(data.settings.announcement || "")}</textarea></label><div class="field-pair"><label class="field">Bảo hành lần giao (ngày)<input name="delivery_warranty_days" type="number" min="0" max="365" value="${operationSettings.deliveryWarrantyDays}" required></label><label class="field">Cảnh báo kho khi còn dưới<input name="low_stock_threshold" type="number" min="0" max="100000" value="${operationSettings.lowStockThreshold}" required></label></div><div class="trial-settings"><h3>Mở trải nghiệm miễn phí</h3><p>Khi tắt, khách phải dùng lượt đã mua hoặc hạn mức gói.</p><label class="trial-toggle"><input name="trial_nftoken_enabled" type="checkbox" ${trial.nftokenEnabled ? "checked" : ""}><span><b>NFToken trải nghiệm</b><small>Cho mọi người tạo link miễn phí mỗi ngày</small></span></label><label class="field">Lượt NFToken / người / ngày<input name="trial_nftoken_limit" type="number" min="0" max="1000" value="${trial.nftokenDailyLimit}" required></label><label class="trial-toggle"><input name="trial_cookie_enabled" type="checkbox" ${trial.cookieEnabled ? "checked" : ""}><span><b>Cookie VIP trải nghiệm</b><small>Cho mọi người rút Cookie VIP miễn phí mỗi ngày</small></span></label><label class="field">Lượt Cookie VIP / người / ngày<input name="trial_cookie_limit" type="number" min="0" max="1000" value="${trial.cookieDailyLimit}" required></label></div><div class="feature-switches">${Object.entries(featureLabels).map(([key,label]) => `<label><input name="feature_${key}" type="checkbox" ${data.settings.features[key] ? "checked" : ""}><span>${label}</span></label>`).join("")}</div><button class="button wide">Lưu cấu hình hệ thống</button></form></div></details>

    <details class="admin-section"><summary><span><i>📦</i><b>Quản lý kho Cookie</b><small>Premium ${s.premiumStock} · Free ${s.freeStock} · cách ly ${s.quarantinedStock || 0}</small></span><em>⌄</em></summary><div class="admin-section-body"><div class="inventory-cards"><article><b>Premium</b><small>${s.premiumStock} khả dụng · ${s.premiumUsed} đã dùng</small><button data-admin-inventory="premium">Thêm vào kho</button><button data-admin-scan="premium">Kiểm tra 100 mục</button><button data-admin-release="premium">Mở cách ly</button><button class="clean" data-admin-cleanup="premium">Dọn mục đã dùng</button></article><article><b>Free</b><small>${s.freeStock} khả dụng · ${s.freeUsed} đã dùng</small><button data-admin-inventory="free">Thêm vào kho</button><button data-admin-scan="free">Kiểm tra 100 mục</button><button data-admin-release="free">Mở cách ly</button><button class="clean" data-admin-cleanup="free">Dọn mục đã dùng</button></article></div><p class="admin-note">Cookie lỗi mạng được cách ly; chỉ Cookie chết thật mới bị loại. Nội dung Cookie không được trả ra API Admin.</p></div></details>
    ...
    <details class="admin-section"><summary><span><i>🎟</i><b>Mã quà tặng & giảm giá</b><small>Tạo giftcode cộng số dư hoặc mã giảm % khi thanh toán</small></span><em>⌄</em></summary><div class="admin-section-body"><button class="button admin-add" data-admin-code-new>＋ Tạo mã</button>${adminRows(data.codes, (code) => { const percentCode = String(code.code_type || "BALANCE").toUpperCase() === "PERCENT"; const summary = percentCode ? `Giảm ${code.percent || 0}% khi thanh toán` : `${formatMoney(code.amount)} cộng số dư`; return `<article class="admin-row"><div><b>${escapeHtml(code.code)}</b><small>${summary} · còn ${code.uses} lượt${code.per_user ? " · mỗi user 1 lần" : ""}</small></div><button data-admin-code="${escapeHtml(code.code)}">Sửa</button></article>`; }, "Chưa có mã.")}</div></details>
    ...
    <details class="admin-section" open><summary><span><i>♢</i><b>Bảo hành tự động</b><small>${s.pendingWarranties || 0} yêu cầu chờ xử lý</small></span><em>⌄</em></summary><div class="admin-section-body">${adminRows(data.warranties, (item) => `<article class="admin-ticket"><header><b>#${item.id} · ID ${item.user_id} · ${escapeHtml(item.kind)}</b><em>${escapeHtml(item.status)}</em></header><p>${escapeHtml(item.reason)}</p><small>${escapeHtml(item.resolution || item.created_at)}</small>${["pending","checking"].includes(item.status) ? `<div><button data-admin-warranty="${item.id}" data-status="approved">Duyệt & hoàn lượt</button><button data-admin-warranty="${item.id}" data-status="rejected">Từ chối</button></div>` : ""}</article>`, "Chưa có yêu cầu bảo hành.")}</div></details>
    <details class="admin-section"><summary><span><i>🛡</i><b>Thiết bị & chống gian lận</b><small>${s.riskUsers || 0} người dùng có cảnh báo trong 30 ngày</small></span><em>⌄</em></summary><div class="admin-section-body">${adminRows(data.riskUsers, (item) => `<article class="admin-row"><div><b>ID ${item.user_id}${item.username ? ` · @${escapeHtml(item.username)}` : ""}</b><small>Risk ${item.risk_score}/100 · ${item.events} sự kiện · ${escapeHtml(item.last_event)}</small></div></article>`, "Chưa có cảnh báo gian lận.")}</div></details>
    ...
    <details class="admin-section"><summary><span><i>♥</i><b>Vận hành & sao lưu</b><small>Uptime ${Math.floor((data.operations?.uptimeSeconds || 0)/60)} phút · DB ${escapeHtml(data.operations?.database || "unknown")}</small></span><em>⌄</em></summary><div class="admin-section-body"><p>Doanh thu hôm nay: <b>${formatMoney(data.metrics?.revenue?.today || 0)}</b></p><p>7 ngày: <b>${formatMoney(data.metrics?.revenue?.week || 0)}</b> · 30 ngày: <b>${formatMoney(data.metrics?.revenue?.month || 0)}</b></p><button class="button wide" data-admin-operations>Chạy kiểm tra vận hành & backup</button></div></details>
    ...
    <details class="admin-section"><summary><span><i>🛡</i><b>Thiết bị người dùng</b><small>${(data.devices || []).length} thiết bị gần nhất · có thể mở lại thiết bị đã thu hồi</small></span><em>⌄</em></summary><div class="admin-section-body">${adminRows(data.devices, (device) => `<article class="admin-row"><div><b>ID ${device.user_id}${device.username ? ` · @${escapeHtml(device.username)}` : ""}</b><small>${escapeHtml(device.label || "Thiết bị")} · ${escapeHtml(device.platform || "Web")} · lần cuối ${escapeHtml(device.last_seen_at || "")}</small></div>${device.revoked ? `<button data-admin-device-restore="${device.id}">Mở lại</button>` : `<em>Đang hoạt động</em>`}</article>`, "Chưa có thiết bị.")}</div></details>
  </div>`;
}
...
function codeDialog(code = null) {
  const codeType = String(code?.code_type || "PERCENT").toUpperCase();
  const startsAt = code?.starts_at ? String(code.starts_at).slice(0, 16) : "";
  const endsAt = code?.ends_at ? String(code.ends_at).slice(0, 16) : "";
  modal(`<div class="eyebrow">MÃ KHUYẾN MÃI</div><h2>${code ? "Sửa mã" : "Tạo mã mới"}</h2><form data-code-form><label class="field">Tên mã<input name="code" maxlength="50" value="${escapeHtml(code?.code || "")}" ${code ? "readonly" : ""} required></label><label class="field">Loại mã<select name="codeType"><option value="PERCENT" ${codeType === "PERCENT" ? "selected" : ""}>Giảm giá khi thanh toán (%)</option><option value="BALANCE" ${codeType === "BALANCE" ? "selected" : ""}>Giftcode cộng số dư</option></select></label><div class="field-pair"><label class="field">Giảm %<input name="percent" type="number" min="0" max="100" value="${code?.percent || 0}"></label><label class="field">Số tiền cộng<input name="amount" type="number" min="0" value="${code?.amount || 0}"></label></div><div class="field-pair"><label class="field">Lượt sử dụng<input name="uses" type="number" min="1" value="${code?.uses || 1}" required></label><label class="field">Đơn tối thiểu<input name="minOrderTotal" type="number" min="0" value="${code?.min_order_total || 0}"></label></div><label><input name="perUser" type="checkbox" ${code?.per_user !== 0 ? "checked" : ""}> Mỗi user chỉ dùng một lần</label><label class="field">Bắt đầu (không bắt buộc)<input name="startsAt" type="datetime-local" value="${escapeHtml(startsAt)}"></label><label class="field">Kết thúc (không bắt buộc)<input name="endsAt" type="datetime-local" value="${escapeHtml(endsAt)}"></label><label class="field">Sản phẩm áp dụng<input name="productIds" value="${escapeHtml(code?.product_ids || "")}" placeholder="Để trống = tất cả; nhiều ID cách nhau bằng dấu phẩy"></label><button class="button wide">Lưu mã</button></form>`, {onOpen(root, close) { root.querySelector("[data-code-form]").onsubmit = async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const value = {amount: Number(form.get("amount") || 0), uses: Number(form.get("uses") || 0), codeType: form.get("codeType"), percent: Number(form.get("percent") || 0), perUser: form.has("perUser"), startsAt: form.get("startsAt"), endsAt: form.get("endsAt"), minOrderTotal: Number(form.get("minOrderTotal") || 0), productIds: String(form.get("productIds") || "").trim()}; try { await api.adminSaveCode(form.get("code"), value); close(); await loadAdmin(); toast("Đã lưu mã"); } catch (error) { toast(error.message, "error"); } }; }});
}
...
export function bindAdminEvents() {
  ...
  document.querySelector("[data-admin-settings]")?.addEventListener("submit", async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const features = {}; ["tv","planToken","vipToken","freeCookie","giftcode","deposit","support"].forEach((key) => { features[key] = form.has(`feature_${key}`); }); const trial = {nftokenEnabled: form.has("trial_nftoken_enabled"), nftokenDailyLimit: Number(form.get("trial_nftoken_limit")), cookieEnabled: form.has("trial_cookie_enabled"), cookieDailyLimit: Number(form.get("trial_cookie_limit"))}; const operations = {deliveryWarrantyDays:Number(form.get("delivery_warranty_days")),lowStockThreshold:Number(form.get("low_stock_threshold"))}; try { await api.adminSaveSettings({maintenance: form.has("maintenance"), announcement: form.get("announcement"), features, trial, operations}); await loadAdmin(); toast("Đã lưu cấu hình hệ thống"); } catch (error) { toast(error.message, "error"); } });
  ...
  document.querySelectorAll("[data-admin-scan]").forEach((button) => button.onclick = async () => { const restore = busyButton(button, "Đang tạo job..."); try { const result = await api.adminScanInventory(button.dataset.adminScan, 100); if (!result.job_id) { toast("Kho không có mục cần kiểm tra"); return; } toast(`Đang kiểm tra ${result.total} Cookie ở nền`); const poll = async () => { try { const job = await api.request(`/api/admin/inventory/job/${result.job_id}`); if (job.status === "done") { await loadAdmin(); toast(`Kho: ${job.result.live} live · ${job.result.dead} chết · ${job.result.quarantined} cách ly`); } else if (job.status === "error") toast(job.result?.error || "Kiểm tra kho thất bại", "error"); else setTimeout(poll, 2000); } catch { setTimeout(poll, 3000); } }; setTimeout(poll, 1500); } catch (error) { toast(error.message, "error"); } finally { restore(); } });
  document.querySelectorAll("[data-admin-release]").forEach((button) => button.onclick = async () => { try { const result = await api.adminReleaseQuarantine(button.dataset.adminRelease); await loadAdmin(); toast(`Đã mở cách ly ${result.released} mục`); } catch (error) { toast(error.message, "error"); } });
  document.querySelectorAll("[data-admin-warranty]").forEach((button) => button.onclick = async () => { const status = button.dataset.status; if (!confirm(translateText(status === "approved" ? "Duyệt bảo hành và hoàn đúng loại lượt cho khách?" : "Từ chối yêu cầu bảo hành này?"))) return; try { await api.adminResolveWarranty(button.dataset.adminWarranty, {status}); await loadAdmin(); toast("Đã xử lý bảo hành"); } catch (error) { toast(error.message, "error"); } });
  document.querySelector("[data-admin-operations]")?.addEventListener("click", async (event) => { const restore = busyButton(event.currentTarget, "Đang kiểm tra..."); try { const result = await api.adminOperations(true); state.admin.operations = result; toast("Vận hành và backup đã được kiểm tra"); } catch (error) { toast(error.message, "error"); } finally { restore(); } });
  ...
  document.querySelectorAll("[data-admin-device-restore]").forEach((button) => button.onclick = async () => {
    if (!confirm(translateText("Mở lại thiết bị này cho người dùng?"))) return;
    const restore = busyButton(button, "Đang mở...");
    try { await api.adminRestoreDevice(button.dataset.adminDeviceRestore); await loadAdmin(); toast("Đã mở lại thiết bị"); }
    catch (error) { toast(error.message, "error"); }
    finally { restore(); }
  });
}
